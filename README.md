...
2. Создай токен со scopes: `contact_*`, `task_*`, `common_metadata`, `customfield_add`.
3. В `.env`:
   ```env
   ...
   PLANFIX_ASSIGNEE_USER_ID=1
   ```

Создаётся контакт + элемент объекта «Сделка» (контрагент = контакт).

В поля сделки пишется:
- **Тип экрана**, **Размер экрана**, **Место установки**, **Валюта**, **Статус оплаты**, **Источник лида**, **Ответственный**
- поля квиза из набора **«Квиз LED заявки»**: Квиз, Тип исполнения, Шаг пикселя, Ширина/Высота мм, Монтаж, Страница, MAX, Telegram
- полное описание квиза в тексте сделки

Чтобы поля квиза появились в карточке:
1. **Управление аккаунтом** → **Настраиваемые поля** → **Поля задач** → набор **«Квиз LED заявки»**
2. **Управление аккаунтом** → **Объекты** → **Сделка** → редактировать карточку → **+** → добавить эти 9 полей → сохранить
